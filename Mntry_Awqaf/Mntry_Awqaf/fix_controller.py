...
old_logic = '''            var groupedFields = assignments
                .GroupBy(a => string.IsNullOrEmpty(a.GroupName) ? " " : a.GroupName)
                .Select(g => new
                {
                    groupName = g.Key,
                    fields = g.Select(a => a.DynamicField)
                              .GroupBy(f => f.Id) // Ensure distinct fields in a group
                              .Select(f => f.First())
                              .Select(f => new {
                                  id = f.Id,
                                  name = f.Name,
...
new_logic = '''            var groupedFields = assignments
                .GroupBy(a => string.IsNullOrEmpty(a.GroupName) ? " " : a.GroupName)
                .Select(g => new
                {
                    groupName = g.Key,
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print('DynamicFormsController replaced successfully')
